from PyQt5.QtWidgets import QWidget, QHBoxLayout, QLineEdit, QVBoxLayout, QComboBox,QLabel,QPushButton,QColorDialog,QFileDialog
    ...
    def rowInit(self):
        ...
        for attribute in self.Flicker.__dict__:
            ...
            if isinstance(attr, QColor):
                temp = QWidget()
                temp.mousePressEvent = lambda event,widget=temp,row=self,f=self.Flicker: open_color_chooser(widget,row,f)
...
class FlickerTable(QWidget):
    ...
    def AddNewFlicker(self, flicker):
        ...
        self.VLayout.insertWidget(self.VLayout.count()-1,row)
